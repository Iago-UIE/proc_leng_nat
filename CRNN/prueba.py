...
num_classes = len(characters) + 2  # Valores válidos: 1 a num_classes - 2

# Función para convertir texto a secuencia numérica usando 0 para padding
def text_to_sequence(text, max_len):
    # Convertir cada carácter usando el mapeo
    sequence = [char_to_num.get(char, 0) for char in text]
    # Rellenar con 0 (padding) al final, sin intercalar ceros
    sequence += [0] * (max_len - len(sequence))
    return sequence[:max_len]

# 3. Longitud máxima de etiqueta (sin padding)
max_label_length = max(len(text) for text in y_texts)

# 4. Convertir las etiquetas a secuencias numéricas
...
# Convertir las etiquetas a int32
...
x = tf.keras.layers.MaxPooling2D((2, 2))(x)
# Reestructurar para RNN (secuencia)
x = tf.keras.layers.Reshape((x.shape[1] * x.shape[2], 128))(x)
x = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(128, return_sequences=True))(x)
# Capa de salida: num_classes neuronas
...
def ctc_loss(y_true, y_pred):
    ...
    # Calcular la longitud real contando valores distintos de 0
...
# Para decodificar: ignorar 0 (padding) y el token en blanco (num_classes - 1)
num_to_char = {v: k for k, v in char_to_num.items()}
predicted_text = "".join([num_to_char.get(np.argmax(p), '') 
                          for p in prediccion[0] if np.argmax(p) not in [0, num_classes - 1]])
...
# Suponiendo que 'model' es tu modelo Keras ya entrenado
# Define la firma de entrada: en este caso, imágenes con forma (None, 200, 50, 1)
spec = (tf.TensorSpec((None, 200, 50, 1), tf.float32, name="input"),)

# Convierte el modelo a ONNX, especificando una versión de opset (por ejemplo, 13)
